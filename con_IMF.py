# Cài đặt thư viện PyEMD và fastdtw
...
if __name__ == "__main__":
    ...
    for n in range(nIMFs):
        ...
        print("imf", n+1, '-euclidean distance to original series:', distance, "; corr:", corr, ";p-value:", pval)

        np.savetxt(folder + "/imfs/IMF-" + str(n) + ".csv", eIMFs[n], delimiter=",")
    plt.subplot(nIMFs+2, 1, nIMFs+2)
    plt.plot(time_values, eIMFs[nIMFs], 'g')
    plt.ylabel("Residual")
    ...
    # ## clustering
    # km = KMeans(n_clusters=3, init='random', max_iter=100, n_init=1)
    # km.fit(x)
